 ...
 * @dev Test contract for Smart contract wallets, proxies some calls an EOA wallet would make to setup on stateswap.
 */
contract TestSmartContractWallet {
    ...
    // Called by stateswap when this contract is taker for an order with eth value exchanged.
    ...
    // Proxy to exchange
    function approveOrder_(address exchange, address registry, address maker, address verifierTarget, bytes4 verifierSelector, bytes calldata verifierExtradata, uint maximumFill, uint listingTime, uint expirationTime, uint salt, bool orderbookInclusionDesired)
    public returns (bool) {
        ExchangeInterface(exchange).approveOrder_(registry, maker, verifierTarget, verifierSelector, verifierExtradata, maximumFill, listingTime, expirationTime, salt, orderbookInclusionDesired);
        return true;
    }
