...
/**
 * @title VerifierMarket
 * @author Stateswap Protocol Developers
 */
contract VerifierMarket {

	string public constant name = "Verifier Market";
		...
		returns (uint)
	{
		...
		require(howToCalls[0] == AuthenticatedProxy.HowToCall.Call, "anyERC1155ForERC20: Execution call must be a direct call");
		...
		require(addresses[2] == tokenGiveGet[0], "anyERC1155ForERC20: Execution call target must equal address of token to give");
		require(addresses[5] == tokenGiveGet[1], "anyERC1155ForERC20: Execution countercall target must equal address of token to get");
		...
		returns (uint)
	{
		...
		require(tokenIdAndNumeratorDenominator[1] > 0,"anyERC20ForERC1155: numerator must be larger than zero");
		require(tokenIdAndNumeratorDenominator[2] > 0,"anyERC20ForERC1155: denominator must be larger than zero");
		require(addresses[2] == tokenGiveGet[0], "anyERC20ForERC1155: Execution call target must equal address of token to get");
		require(addresses[5] == tokenGiveGet[1], "anyERC20ForERC1155: Execution countercall target must equal address of token to give");
		...
		returns (uint)
	{
		...
		require(addresses[2] == tokenGiveGet[0], "anyERC20ForERC20: Execution call target must equal address of token to give");
		require(addresses[5] == tokenGiveGet[1], "anyERC20ForERC20: Execution countercall target must equal address of token to get");
		...
		returns (uint)
	{
		...
		require(addresses[2] == tokenGiveGet[0], "ERC721ForERC20: Execution call target must equal address of token to give");
		require(addresses[5] == tokenGiveGet[1], "ERC721ForERC20: Execution countercall target must equal address of token to get");
		...
		returns (uint)
	{
		...
		require(howToCalls[0] == AuthenticatedProxy.HowToCall.Call, "ERC20ForERC721: Execution call must be a direct call");

		(address[2] memory tokenGiveGet, uint256[2] memory tokenIdAndPrice) = abi.decode(extra, (address[2], uint256[2]));

		require(tokenIdAndPrice[1] > 0,"ERC20ForERC721: ERC721 price must be larger than zero");
		require(addresses[2] == tokenGiveGet[0], "ERC20ForERC721: Execution call target must equal address of token to give");
		require(addresses[5] == tokenGiveGet[1], "ERC20ForERC721: Execution countercall target must equal address of token to get");
