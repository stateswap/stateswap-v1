/*
    ...
    VerifierUtil - verifier call utility contract
...
contract VerifierUtil is StaticCaller {
    ...
    {
        /*
           Accept any Effectfull call.
           Useful e.g. for matching-by-transaction, where you authorize the Effectfull counter-call by sending the transaction and don't need to re-check it.
           Return fill "1".
        */
        ...
        returns (uint)
    {
        ...
        /* Split into two static calls: one for the Effectfull call, one for the Effectfull counter-call, both with metadata. */

        /* Static call to check the Effectfull call. */
        require(staticCall(targets[0], abi.encodeWithSelector(selectors[0], firstExtradata, addresses, howToCalls[0], uints, data)));

        /* Static call to check the Effectfull counter-call. */
        ...
        view
    {
        ...
        /* Assert DELEGATECALL to atomicizer library with given Effectfull call sequence, split up predicates accordingly.
        ...
        require(addrs.length == caddrs.length); // Exact Effectfull calls only
        ...
        view
    {
        ...
        require(addresses[2] == atomicizer);
        require(howToCall == AuthenticatedProxy.HowToCall.DelegateCall);
        require(addrs.length <= caddrs.length); // Extra Effectfull calls OK
