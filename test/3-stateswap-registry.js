...
contract('StateswapRegistry', accounts => {
  it('is deployed', async () => {
    return await StateswapRegistry.deployed()
  })

  it('does not allow additional grant', async () => {
    let registry = await StateswapRegistry.deployed()
    return assertIsRejected(
      registry.grantInitialAuthentication(registry.address),
      /Stateswap Protocol Proxy Registry initial address already set/,
  ...
  it('allows proxy revocation', async () => {
    let [registry, testProxy] = await Promise.all([StateswapRegistry.deployed(), TestAuthenticatedProxy.deployed()])
    let proxy = await registry.proxies(accounts[1])
