...
contract('StateswapExchange', (accounts) => {
	let deploy_core_contracts = async () => {
		...
		let [exchange, verifieri] = await Promise.all([StateswapExchange.new(CHAIN_ID, [registry.address], '0x'), StateswapVerifier.new(atomicizer.address)])
	...
	it('matches erc1155 nft-nft swap order', async () => {
		...
		const one = { registry: registry.address, maker: account_a, verifierTarget: verifieri.address, verifierSelector: selector, verifierExtradata: paramsOne, maximumFill: '1', listingTime: '0', expirationTime: '10000000000', salt: '7' }
		const two = { registry: registry.address, maker: account_b, verifierTarget: verifieri.address, verifierSelector: selector, verifierExtradata: paramsTwo, maximumFill: '1', listingTime: '0', expirationTime: '10000000000', salt: '8' }
	...
	it('matches nft-nft swap order, abi-decoding instead', async () => {
		...
		const one = { registry: registry.address, maker: account_a, verifierTarget: verifieri.address, verifierSelector: selector, verifierExtradata: paramsOne, maximumFill: '1', listingTime: '0', expirationTime: '10000000000', salt: '333123' }
		const two = { registry: registry.address, maker: account_b, verifierTarget: verifieri.address, verifierSelector: selector, verifierExtradata: paramsTwo, maximumFill: '1', listingTime: '0', expirationTime: '10000000000', salt: '123344' }
	...
	it('matches erc1155 + erc20 <> erc1155 orders, matched left, real static call', async () => {
		...
		let { atomicizer, exchange, registry, verifieri } = await deploy_core_contracts()
		...
		const one = { registry: registry.address, maker: account_a, verifierTarget: verifieri.address, verifierSelector: selectorOne, verifierExtradata: extradataOne, maximumFill: '1', listingTime: '0', expirationTime: '10000000000', salt: '3352' }
		const two = { registry: registry.address, maker: account_b, verifierTarget: verifieri.address, verifierSelector: selectorTwo, verifierExtradata: extradataTwo, maximumFill: '1', listingTime: '0', expirationTime: '10000000000', salt: '3335' }
		const sig = NULL_SIG
	...
	const erc1155_erc20_match_right_verifier_call = async (maximumFill, fillCount) => {
		let account_a = accounts[0]
		let account_b = accounts[6]
		...
		const extradataOneA = web3.eth.abi.encodeParameters(
			['address[]', 'uint256[]', 'bytes4[]', 'bytes'],
			[[verifieri.address], [(aEDParams.length - 2) / 2], [aEDSelector], aEDParams]
		)
		...
		const extradataTwo = '0x'
		const one = { registry: registry.address, maker: account_a, verifierTarget: verifieri.address, verifierSelector: selectorOne, verifierExtradata: extradataOne, maximumFill: '2', listingTime: '0', expirationTime: '10000000000', salt: '3358' }
		const two = { registry: registry.address, maker: account_b, verifierTarget: verifieri.address, verifierSelector: selectorTwo, verifierExtradata: extradataTwo, maximumFill: '1', listingTime: '0', expirationTime: '10000000000', salt: '3339' }
		//const twob = {registry: registry.address, maker: account_b, verifierTarget: verifieri.address, verifierSelector: selectorTwo, verifierExtradata: extradataTwo, maximumFill: '1', listingTime: '0', expirationTime: '10000000000', salt: '3340'}
		...
		for (let i = 0; i < fillCount; ++i)
			await exchange.stateswapWith(one, sig, firstCall, two, twoSig, secondCall, ZERO_BYTES32, { from: account_b })
	...
	it('matches erc1155 <> erc20 signed orders, matched right, real verifier call', async () => {
		return erc1155_erc20_match_right_verifier_call(1, 1)
	})

	it('matches erc1155 <> erc20 signed orders, matched right, real verifier call, multiple fills', async () => {
		return erc1155_erc20_match_right_verifier_call(2, 2)
	})
