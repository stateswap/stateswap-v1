...
const VerifierMarket = artifacts.require('VerifierMarket')
...
contract('StateswapExchange', (accounts) => {
	let deploy_core_contracts = async () => {
		let [registry, atomicizer] = await Promise.all([StateswapRegistry.new(), StateswapAtomicizer.new()])
		let [exchange, verifieri] = await Promise.all([StateswapExchange.new(CHAIN_ID, [registry.address], '0x'), VerifierMarket.new()])
		await registry.grantInitialAuthentication(exchange.address)
		return { registry, exchange: wrap(exchange), atomicizer, verifieri }
	}
	...
	const any_erc1155_for_erc20_test = async (options) => {
		...
		let { exchange, registry, verifieri } = await deploy_core_contracts()
		...
		const one = { registry: registry.address, maker: account_a, verifierTarget: verifieri.address, verifierSelector: selectorOne, verifierExtradata: paramsOne, maximumFill: (sellingNumerator || 1) * sellAmount, listingTime: '0', expirationTime: '10000000000', salt: '11' }
		const two = { registry: registry.address, maker: account_b, verifierTarget: verifieri.address, verifierSelector: selectorTwo, verifierExtradata: paramsTwo, maximumFill: buyingPrice * buyAmount, listingTime: '0', expirationTime: '10000000000', salt: '12' }
		...
		for (var i = 0; i < txCount; ++i) {
			...
			await exchange.stateswapWith(one, sigOne, firstCall, two, sigTwo, secondCall, ZERO_BYTES32, { from: sender || account_a })
			two.salt++
		}
	...
	it('VerifierMarket: matches erc1155 <> erc20 order, 1 fill', async () => {
		const price = 10000
	...
	it('VerifierMarket: matches erc1155 <> erc20 order, multiple fills in 1 transaction', async () => {
		const amount = 3
		const price = 10000
	...
	it('VerifierMarket: matches erc1155 <> erc20 order, multiple fills in multiple transactions', async () => {
		const nftAmount = 3
		const buyAmount = 1
		const price = 10000
	...
	it('VerifierMarket: matches erc1155 <> erc20 order, allows any partial fill', async () => {
		const nftAmount = 30
		const buyAmount = 4
		const price = 10000
	...
	it('VerifierMarket: matches erc1155 <> erc20 order with any matching ratio', async () => {
		const lot = 83974
		const price = 972
	...
	it('VerifierMarket: does not match erc1155 <> erc20 order beyond maximum fill', async () => {
		const price = 10000
	...
	it('VerifierMarket: does not fill erc1155 <> erc20 order with different prices', async () => {
		const price = 10000
	...
	it('VerifierMarket: does not fill erc1155 <> erc20 order with different ratios', async () => {
		const price = 10000
	...
	it('VerifierMarket: does not fill erc1155 <> erc20 order beyond maximum sell amount', async () => {
		const nftAmount = 2
		const buyAmount = 3
		const price = 10000
	...
	it('VerifierMarket: does not fill erc1155 <> erc20 order if balance is insufficient', async () => {
		const nftAmount = 1
		const buyAmount = 1
		const price = 10000
	...
	it('VerifierMarket: does not fill erc1155 <> erc20 order if the token IDs are different', async () => {
		const price = 10000
	...
	const any_erc20_for_erc20_test = async (options) => {
		...
		const one = { registry: registry.address, maker: account_a, verifierTarget: verifieri.address, verifierSelector: selector, verifierExtradata: paramsOne, maximumFill: sellAmount, listingTime: '0', expirationTime: '10000000000', salt: '11' }
		const two = { registry: registry.address, maker: account_b, verifierTarget: verifieri.address, verifierSelector: selector, verifierExtradata: paramsTwo, maximumFill: txCount * sellingPrice * buyAmount, listingTime: '0', expirationTime: '10000000000', salt: '12' }
	...
	it('VerifierMarket: matches erc20 <> erc20 order, 1 fill', async () => {
		const price = 10000
	...
	it('VerifierMarket: matches erc20 <> erc20 order, multiple fills in 1 transaction', async () => {
		const amount = 3
		const price = 10000
	...
	it('VerifierMarket: matches erc20 <> erc20 order, multiple fills in multiple transactions', async () => {
		const sellAmount = 3
		const buyAmount = 1
		const price = 10000
		const transactions = 3
	...
	it('VerifierMarket: matches erc20 <> erc20 order, allows any partial fill', async () => {
		const sellAmount = 30
		const buyAmount = 4
		const price = 10000
	...
	it('VerifierMarket: does not match erc20 <> erc20 order beyond maximum fill', async () => {
		const price = 10000
	...
	it('VerifierMarket: does not fill erc20 <> erc20 order with different taker price', async () => {
		const price = 10000
	...
	it('VerifierMarket: does not fill erc20 <> erc20 order beyond maximum sell amount', async () => {
		const sellAmount = 2
		const buyAmount = 3
		const price = 10000
	...
	it('VerifierMarket: does not fill erc20 <> erc20 order if balance is insufficient', async () => {
		const sellAmount = 1
		const buyAmount = 1
		const price = 10000
	...
	const erc721_for_erc20_test = async (options) => {
		...
		const one = { registry: registry.address, maker: account_a, verifierTarget: verifieri.address, verifierSelector: selectorOne, verifierExtradata: paramsOne, maximumFill: 1, listingTime: '0', expirationTime: '10000000000', salt: '11' }
		const two = { registry: registry.address, maker: account_b, verifierTarget: verifieri.address, verifierSelector: selectorTwo, verifierExtradata: paramsTwo, maximumFill: 1, listingTime: '0', expirationTime: '10000000000', salt: '12' }
	...
	it('VerifierMarket: matches erc721 <> erc20 order', async () => {
		const price = 15000
	...
	it('VerifierMarket: does not fill erc721 <> erc20 order with different prices', async () => {
		const price = 15000
	...
	it('VerifierMarket: does not fill erc721 <> erc20 order if the balance is insufficient', async () => {
		const price = 15000
	...
	it('VerifierMarket: does not fill erc721 <> erc20 order if the token IDs are different', async () => {
		const price = 15000
