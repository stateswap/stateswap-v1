...
const wrap = (inst) => {
  var obj = {
    inst: inst,
    hashOrder: (order) => inst.hashOrder_.call(order.registry, order.maker, order.verifierTarget, order.verifierSelector, order.verifierExtradata, order.maximumFill, order.listingTime, order.expirationTime, order.salt),
    hashToSign: (order) => {
      return inst.hashOrder_.call(order.registry, order.maker, order.verifierTarget, order.verifierSelector, order.verifierExtradata, order.maximumFill, order.listingTime, order.expirationTime, order.salt).then(hash => {
        return inst.hashToSign_.call(hash)
      })
    },
    validateOrderParameters: (order) => inst.validateOrderParameters_.call(order.registry, order.maker, order.verifierTarget, order.verifierSelector, order.verifierExtradata, order.maximumFill, order.listingTime, order.expirationTime, order.salt),
    ...
    approveOrder: (order, inclusion, misc) => inst.approveOrder_(order.registry, order.maker, order.verifierTarget, order.verifierSelector, order.verifierExtradata, order.maximumFill, order.listingTime, order.expirationTime, order.salt, inclusion, misc),
    setOrderFill: (order, fill) => inst.setOrderFill_(hashOrder(order), fill),
    stateswap: (order, sig, call, counterorder, countersig, countercall, metadata) => inst.stateswap_(
    ...
    stateswapWith: (order, sig, call, counterorder, countersig, countercall, metadata, misc) => inst.stateswap_(
      [order.registry, order.maker, order.verifierTarget, order.maximumFill, order.listingTime, order.expirationTime, order.salt, call.target,
      counterorder.registry, counterorder.maker, counterorder.verifierTarget, counterorder.maximumFill, counterorder.listingTime, counterorder.expirationTime, counterorder.salt, countercall.target],
      [order.verifierSelector, counterorder.verifierSelector],
      order.verifierExtradata, call.data, counterorder.verifierExtradata, countercall.data,
